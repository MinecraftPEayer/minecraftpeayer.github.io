...
const DIFF_NAMES: Record<string, string> = {
  "1": "EASY", "2": "BASIC", "3": "ADVANCED", "4": "EXPERT", "5": "MASTER", "6": "Re:MASTER"
};

// --- 自訂播放器元件 ---
const CustomAudioPlayer = ({ src, duration, filename, title }: { src: string, duration: number, filename: string, title: string }) => {
  ...
  return (
    <div className="p-4 bg-gray-50 rounded-lg border">
      ...
      <div className="flex items-center gap-3">
        <button onClick={togglePlay} className="flex-shrink-0 w-10 h-10 flex items-center justify-center bg-blue-600 text-white rounded-full hover:bg-blue-700 transition">
          {isPlaying ? '⏸' : '▶'}
        </button>
        <span className="text-sm font-mono w-10 text-right text-gray-600">{formatTime(currentTime)}</span>
        <input 
          type="range" 
          min="0" 
          max={duration || 100} 
        ...
        />
        <span className="text-sm font-mono w-10 text-gray-600">{formatTime(duration)}</span>
      </div>
      <a href={src} download={filename} className="mt-4 block text-center py-2 bg-green-600 hover:bg-green-700 text-white rounded-md font-bold transition">
        📥 下載 MP3
      </a>
    </div>
  );
...
export default function MaimaiAudioTool() {
  ...
  const [results, setResults] = useState<{ withBgm?: string; withoutBgm?: string; filenames: string[] }>({ filenames: [] });
  ...
  const handleFolderSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (name.startsWith('track.') && /\.(mp3|wav|ogg)$/.test(name)) foundSong = f;
      if (name === 'maidata.txt') foundMaida = f;
    }
    if (foundSong && songFileRef.current) {
      const dt = new DataTransfer(); dt.items.add(foundSong);
    ...
    else if (foundSong || foundMaida) setStatus(`⚠️ 僅找到 ${foundSong ? foundSong.name : foundMaida!.name}，請手動補齊。`);
  ...
  const parseSimaiTime = (chartString: string) => {
    ...
    for (let chunk of chunks) {
      ...
      holdMatches.forEach(m => {
        const content = m[1];
        let duration = 0;
        if (content.startsWith('#')) duration = parseFloat(content.substring(1));
    ...
    return [...new Set(rawTimes.map(t => Math.round(t * 10000) / 10000))].sort((a, b) => a - b);
  };
  ...
  const startProcess = async () => {
    ...
    try {
      ...
      await updateProgress(5, "⚙️ 正在解碼音訊...");
      
      const ctx = new AudioContext();
      const songBuf = await ctx.decodeAudioData(await songFile.arrayBuffer());
      const hitBuf = await ctx.decodeAudioData(await hitFile.arrayBuffer());
      ...
      // 1. With BGM
      const ctxWith = new OfflineAudioContext(songBuf.numberOfChannels, songBuf.length, songBuf.sampleRate);
      ...
      for (let i = 0; i < totalNotes; i += noteChunk) {
        ...
        await updateProgress(10 + (end / totalNotes) * 5, `準備含BGM圖譜 (${end}/${totalNotes})`);
      }

      await updateProgress(15, "正在渲染含BGM音軌...");
      const withBuf = await ctxWith.startRendering();
      const withBlob = await bufferToMp3Async(withBuf, async (current, total) => {
        await updateProgress(15 + (current / total) * 35, `壓縮含BGM的 MP3 (${current}/${total})`);
      });
      ...
      for (let i = 0; i < totalNotes; i += noteChunk) {
        ...
        await updateProgress(50 + (end / totalNotes) * 5, `準備無BGM圖譜 (${end}/${totalNotes})`);
      }

      await updateProgress(55, "正在渲染無BGM音軌...");
      const withoutBuf = await ctxWithout.startRendering();
      const withoutBlob = await bufferToMp3Async(withoutBuf, async (current, total) => {
        await updateProgress(55 + (current / total) * 45, `壓縮無BGM的 MP3 (${current}/${total})`);
      });

      setResults({
        withBgm: URL.createObjectURL(withBlob),
        withoutBgm: URL.createObjectURL(withoutBlob),
        filenames: [`${songTitle}_${diffLabel}_with_BGM.mp3`, `${songTitle}_${diffLabel}_without_BGM.mp3`]
      });
  ...
  return (
    <div className="max-w-3xl mx-auto p-8 bg-white shadow-xl rounded-2xl my-10">
      ...
      <h1 className="text-3xl font-bold text-center text-blue-600 mb-8">Maimai 譜面預覽合成器</h1>

      <div className="mb-6 p-5 bg-blue-50 border-2 border-dashed border-blue-300 rounded-xl">
        <label className="block text-blue-800 font-bold mb-2 text-lg">📁 快速資料夾匯入</label>
        <input 
          type="file" 
        ...
        />
        <p className="text-sm text-gray-500 mt-2">（自動尋找資料夾中的 <b>track.mp3/wav/ogg</b> 與 <b>maidata.txt</b>）</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div className="flex flex-col">
          <label className="font-semibold text-gray-700 mb-1">1. 歌曲音檔</label>
          <input type="file" ref={songFileRef} onChange={handleSongChange} accept="audio/*" className="border p-2 rounded-lg w-full bg-gray-50" />
        </div>
        <div className="flex flex-col">
          <label className="font-semibold text-gray-700 mb-1">2. maidata.txt</label>
          <input type="file" ref={maidaFileRef} onChange={handleMaidataChange} accept=".txt" className="border p-2 rounded-lg w-full bg-gray-50" />
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <div className="flex flex-col">
          <label className="font-semibold text-gray-700 mb-1">3. 正解音 (Tap Sound)</label>
          <input type="file" ref={hitFileRef} accept="audio/*" className="border p-2 rounded-lg w-full bg-gray-50" />
        </div>
        <div className="flex flex-col">
          <label className="font-semibold text-gray-700 mb-1">4. 選擇難度</label>
          <select ref={diffSelectRef} className="border p-2 rounded-lg w-full bg-white">
            {Object.keys(chartData).length === 0 ? (
              <option value="">請先讀取譜面...</option>
            ) : (
              Object.keys(chartData).map(key => (
                <option key={key} value={key}>{DIFF_NAMES[key.split('_')[1]] || key}</option>
              ))
            )}
          </select>
        </div>
      ...
      {(progress > 0 || isProcessing) && (
        <div className="mt-6">
          ...
          <p className="text-center mt-2 font-bold text-orange-600">{status}</p>
        </div>
      )}

      {results.withBgm && results.withoutBgm && (
        <div className="mt-8 space-y-4 border-t pt-8">
          <CustomAudioPlayer src={results.withBgm} duration={songDuration} filename={results.filenames[0]} title="🎵 包含背景音樂 (With BGM) - MP3" />
          <CustomAudioPlayer src={results.withoutBgm} duration={songDuration} filename={results.filenames[1]} title="🥁 僅正解音 (Without BGM) - MP3" />
        </div>
      )}
